...
for TRY = 1:S.nTries
    ...
    S.p = p0;
    [p,~] = fminsearch(@(p) costLeg(p,S), S.p); % OPTIMIZATION
    S.p = p;
    [p,c] = fminsearch(@(p) costLeg(p,S), S.p);  % OPTIMIZATION
...
fprintf('\n\n\n\n Final Optimization~~\n')
[p,c] = fminsearch(@(p) costLeg(p,S),S.bestP); % last and best OPTIMIZATION
if c < S.bestCost
    S.bestCost = c;
    S.bestP = p;
    fprintf(' c=%g, ',c); p'
...
S.p = S.bestP;
[c,meanErr] = costLeg(S.p, S); meanErr
